...
def detect():
    try:
        if len(torch.tensor(results.xyxy[0])) <= 1 and int(torch.tensor(results.xyxy[0])[0][5].item()) == steps_count[steps]:
            ...
            return (str(nn_new_x) +" "+ str(new_y) +" "+ str(steps_count[steps]))
                ...
                #print("continuous xandy for:",steps_count[steps]  ,nn_new_x, new_y)
                if steps_count[steps] == 1:
                    return (str(nn_new_x) +" "+ str(new_y) +" "+ (str(steps_count[steps])+ "_" + str(int(torch.tensor(results.xyxy[0])[0][1].item())) + "_" + str(int(torch.tensor(results.xyxy[0])[0][3].item()))) )
                return (str(nn_new_x) +" "+ str(new_y) +" "+ str(steps_count[steps]))
